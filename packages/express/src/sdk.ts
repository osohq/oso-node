      ...
      switch (req.method.toLowerCase()) {
        ...
        default: {
          throw new OsoSdkError(
            `method ${req.method.toLowerCase()} is not supported`
          );
        }
      }
    };
  ...
  enforce = (args: Enforce) => {
    return async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const resourceType =
          typeof args.resourceType === 'function'
            ? args.resourceType(req)
            : args.resourceType;

        const resourceId =
          typeof args.resourceId === 'function'
            ? args.resourceId(req)
            : tryPathParam(req, args.resourceId);

        const result = await this.authorize(
          { type: 'User', id: await this.actorIdFromRequest(req) },
          await action,
          {
